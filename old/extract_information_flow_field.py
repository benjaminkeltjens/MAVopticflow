...
def determine_optical_flow(prev_bgr, bgr, graphics= True):
    ...
    time_grey_st = time.time()
    prev_gray = cv2.cvtColor(prev_bgr, cv2.COLOR_BGR2GRAY);
    gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY);
    print('time for grey' + str(time.time() - time_grey_st))
    ...
    # detect features:
    time_features_st = time.time()
    points_old = cv2.goodFeaturesToTrack(prev_gray, mask = None, **feature_params);
    print('time for features' + str(time.time() - time_grey_st))
    # calculate optical flow
    time_optical_flow_st = time.time()
    points_new, status, error_match = cv2.calcOpticalFlowPyrLK(prev_bgr, bgr, points_old, None, **lk_params)
    print('time for flow' + str(time.time() - time_optical_flow_st))
...
def show_flow(image_name_1, image_name_2):
    prev_bgr = cv2.imread(image_name_1);
    
    # plt.figure();
    # plt.imshow(prev_bgr);
    # plt.title('First image, nr' );
    
    bgr = cv2.imread(image_name_2);
    
    # plt.figure();
    # plt.imshow(bgr);
    # plt.title('Second image, nr' );
